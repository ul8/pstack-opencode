---
description: Comment-only review subagent (Comment Sicko). Deletes commentary; never edits application code.
mode: subagent
permission:
  edit: deny
  write: deny
...
- Read-only: `read`, `grep`, `glob` only. No `edit` / `write` / `bash`.
