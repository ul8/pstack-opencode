...
- **Worker / verifier.** Spawn via `task` with agent `poteto-agent` or `general`. Prefer an isolated worktree per writer. Use a local child when the worker needs this machine: browser MCP, `bash` verify, simulators, local auth, or OpenCode session data under `$XDG_DATA_HOME/opencode` or `~/.local/share/opencode`. Isolation is the host's job. Do not pass foreign host-isolation fields. Briefs must stand alone (goal, scope, acceptance, verify, report). Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe a clarifying question state.
...
- Exactly one stacker per stack may run `gt` when Graphite is installed, serialized within its stack; record the holder in the standing orders. If `gt` is missing, use `gh` + git and report the dropout. Restacks run in an isolated worktree; restacking the interactive clone at this scale stalls the session.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and child session status. Transcript mtime is not liveness.
