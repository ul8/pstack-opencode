...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the host verify path (bash + browser MCP if present), the host browser / verify path, or the targeted tests. State the outcome of each check, not only the command name.
...
**Readiness.** Open every PR ready, never as a draft. If a PR still opens as a draft, run `gh pr ready <number>`. Run `gh pr view <number>` before you refer to PR status.
