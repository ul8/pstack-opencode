...
Look recursively for `.opencode/skills/**/*-mode/SKILL.md` and `~/.config/opencode/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.opencode/skills/<handle>/`), not only at the top level. If one exists, confirm intent with a plain clarifying question (or the host `question` tool if available) (unless they already said "update my skill" or similar):
...
Locate OpenCode session data under `$XDG_DATA_HOME/opencode` or `~/.local/share/opencode` before fanning out. Scope to this workspace. Do not glob other projects. Do not use Cursor `agent-transcripts` paths.
...
Mining misses intent that hasn't come up yet. Use the a plain clarifying question (or the host `question` tool if available) tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
Follow `playbooks/authoring-a-skill.md` to author the skill. Placement:
...
- Frontmatter formatting: YAML frontmatter with `name` and `description`. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
- Mode skills are heavy and opinionated; they should only apply when the user explicitly invokes them (by name or slash command), not auto-trigger on description matching. Opt out only if the user explicitly wants their mode to apply on every turn.

### 5. Iterate on prose

Apply the **unslop** skill and the authoring-a-skill writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
A `-mode` skill is subjective output. A description-optimization loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): `playbooks/authoring-a-skill.md` alone, no mining required.
