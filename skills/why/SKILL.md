...
Before spawning investigators, list the MCP servers configured for this OpenCode host. Use the available-tools map when present. Otherwise inspect the host's MCP config (project `opencode.json` / `~/.config/opencode/`).
...
Subagent config (each), via the `task` tool:
- agent: `general` (needs MCP; do not use `explore`, which is file-readonly)
- `model`: your configured why-investigators model (default `grok-4.6-fast-xhigh`)
- Investigators must not write files. That is a posture, not a sandbox. If `task` is denied, run serial in-parent and report the dropout.
...
Spawn one synthesizer via the `task` tool:

- agent: `general` (MCP needed to spot-verify citations)
